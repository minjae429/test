...
> **wrist 모터가 화면에 표시되지 않는 현상**: 캘리브레이션 중 wrist 관절이 시각화 화면에 나타나지 않을 수 있습니다. LeRobot 라이브러리 업데이트 과정에서 발생한 버그로 추측되며, **화면 표시 문제일 뿐 실제 동작은 정상**입니다. 그대로 진행하면 됩니다.
...
| 카메라 2대 동시 연결 시 에러 | USB 대역폭 부족으로 두 카메라를 같은 포트 계열에 연결하면 충돌 | 카메라 1대는 노트북 직결, 나머지 1대는 USB 허브를 거쳐 연결 |
